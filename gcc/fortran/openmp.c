...
gfc_match_omp_clauses (gfc_omp_clauses **cp, omp_mask mask,
		       omp_mask dtype_mask, bool first = true,
...
#define OACC_PARALLEL_CLAUSES \
  (omp_mask (OMP_CLAUSE_IF) | OMP_CLAUSE_ASYNC | OMP_CLAUSE_NUM_GANGS	      \
   ...
   | OMP_CLAUSE_WAIT | OMP_CLAUSE_DEVICE_TYPE | OMP_CLAUSE_GNU_PERFECT)
...
static match
match_acc (gfc_exec_op op, const omp_mask mask, const omp_mask dmask)
{
  gfc_omp_clauses *c;
  ...
  if (gfc_match (" ( %n )", n) != MATCH_YES)
      ...
      if (gfc_match_omp_eos () != MATCH_YES)
	{
	  ...
	  return MATCH_ERROR;
				  ...
				  omp_mask (OMP_MASK2_LAST)) != MATCH_YES)
    return MATCH_ERROR;
...
gfc_match_omp_cancel (void)
{
  ...
  if (kind == OMP_CANCEL_UNKNOWN)
    return MATCH_ERROR;
  if (gfc_match_omp_clauses (&c, OMP_CLAUSE_IF, OMP_MASK2_LAST, false)
      != MATCH_YES)
    return MATCH_ERROR;
  c->cancel = kind;
...
gfc_match_omp_end_single (void)
{
  ...
  if (gfc_match_omp_clauses (&c, OMP_CLAUSE_COPYPRIVATE, OMP_MASK2_LAST)
      != MATCH_YES)
    return MATCH_ERROR;
