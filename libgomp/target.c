...
gomp_map_fields_existing (struct target_mem_desc *tgt, splay_tree_key n,
			  ...
			  struct gomp_coalesce_buf *cbuf, bool acc_dc)
{
  struct gomp_device_descr *devicep = tgt->device_descr;
  struct splay_tree_s *mem_map = &devicep->mem_map;
  struct splay_tree_key_s cur_node;
...
{
  if (tgt->list[i].key != NULL && !tgt->list[i].attach)
	       ...
	       bool short_mapkind, enum gomp_map_vars_kind pragma_kind)
{
  ...
  const int typemask = short_mapkind ? 0xff : 0x7;
  struct splay_tree_s *mem_map = &devicep->mem_map;
  struct splay_tree_s *field_map = &devicep->field_map;
  struct splay_tree_s *attach_map = &devicep->attach_map;
  struct splay_tree_key_s cur_node;
  struct target_mem_desc *tgt
	  ...
	  if (openacc)
	    {
	      ...
	      splay_tree_key n = splay_tree_lookup (attach_map, &cur_node);
	      if (n == NULL)
		not_found_cnt++;
	    }
	...
	if (tgt->list[i].key == NULL)
	  {
		...
		first = i + 1;
		last = i + sizes[i];
		if ((get_kind (short_mapkind, kinds, i + 1) & 0xff)
		    == GOMP_MAP_ACC_STRUCT)
		  continue;
		cur_node.host_start = (uintptr_t) hostaddrs[i];
		cur_node.host_end = (uintptr_t) hostaddrs[last]
		...
		if (tgt->list[first].key != NULL)
		  continue;
		n = splay_tree_lookup (mmap, &cur_node);
		if (n == NULL)
		  {
		    size_t align = (size_t) 1 << (kind >> rshift);
		...
		if (openacc)
		  {
		    m = splay_tree_lookup (attach_map, &cur_node);
		    if (m == NULL)
		      {
			tgt->list[i].offset = OFFSET_ACC_POINTER;
			if (splay_tree_lookup (attach_map, &cur_node) == NULL)
			  {
			    array->left = NULL;
			    array->right = NULL;
			    m = &array->key;
			    m->host_start = cur_node.host_start;
			    m->host_end = cur_node.host_end;
			    m->refcount = 0;
			    m->dynamic_refcount = 0;
			    splay_tree_insert (&devicep->attach_map, array);
			    array++;
			    tgt->list[i].key = m;
			    tgt->list[i].attach = true;
			  }
		      }
		  }
	    ...
	    if ((kind & typemask) == GOMP_MAP_ACC_STRUCT)
	      {
		k->host_start = (uintptr_t) hostaddrs[i - 1];
		k->host_end = k->host_start + sizes[i];
		field_tgt_base = k->host_start;
		acc_dc = i + sizes[i - 1] - 1;
	      }
	    else if (!GOMP_MAP_POINTER_P (kind & typemask))
	      k->host_end = k->host_start + sizes[i];
	    else
	      k->host_end = k->host_start + sizeof (void *);
	    splay_tree_key n = splay_tree_lookup (mmap, k);
		...
		if (n && n->refcount == REFCOUNT_LINK)
		  {
		    ...
		    splay_tree_remove (mmap, n);
		    k->link_key = n;
		  }
		size_t align = (size_t) 1 << (kind >> rshift);
		tgt->list[i].key = k;
		k->tgt = tgt;
		// Need to take into account struct fields here
		splay_tree_key m = splay_tree_lookup (field_map, k);
		debug_splay_tree_key (n);
		debug_splay_tree_key (m);
		...
		else if (acc_dc && (kind & typemask) != GOMP_MAP_ACC_STRUCT)
		  {
		    /* populate struct field offset  */
		    k->tgt_offset = k->host_start - field_tgt_base;
		  }
		else
		  {
...
gomp_unmap_vars (struct target_mem_desc *tgt, bool do_copyfrom)
{
  ...
  for (i = 0; i < tgt->list_count; i++)
	...
	continue;
      ...
      if (tgt->list[i].attach && tgt->list[i].offset == OFFSET_ACC_POINTER)
	{
	  k->refcount--;
	  if (k->refcount == 0)
	    {
	      splay_tree_remove (&devicep->attach_map, k);
	      free (k);
	    }
	  continue;
	}
